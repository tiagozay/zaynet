...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === "GET") {
    ...
    try {
        ...
        $solicitacoesDeAmizadeRepository = $entityManager->getRepository(SolicitacaoDeAmizade::class);

        $solicitacoesDeAmizade = $solicitacoesDeAmizadeRepository->findBy(['usuario' => $idUsuario]);
        ...
        $response =  new APIResponse(
            false,
            false,
            "Erro inesperado ao buscar solicitações de amizade",
        );
        echo json_encode($response);
    }
}
